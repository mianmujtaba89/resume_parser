...
const routes = [
  ...
  {
    path: "/ApplicantAppliedJob",
    name: "Application",
    icon: ionicIosPersonAdd,
  },
  
  {
    path: "/ApplicantJobInterview",
    name: "Interviews",
    icon: featherCalendar,
  },
  {
    path: "/ApplicantJobOffer",
    name: "Offering",
    icon: materialGroup,
  },
];

export default function ApplicantSideMenuBar({ children, props }) {
  ...
  const clearUserDetails = async (e) => {
    ...
    const data = await fetch("http://127.0.0.1:8000/api/logout/", {
      ...
      headers: {
        ...
        "Authorization": `Bearer ${localStorage.getItem('applicant-token')}`,
